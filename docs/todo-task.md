...
| # | Task | Status | Notes |
|---|------|--------|-------|
| 1 | Compare `recent_sales_bucket` between `fct_cmis_market_share` and `int_cmis__recent_sales` | Not started / Under review | Verify the calculation logic. Note the ingestion pipeline delay and that `dim_cmis_week` generates a full week for each SKU. |
| 2 | Refactor the `orchestration` folder | Not started | Restructure for maintainability, review dependencies and naming. |
